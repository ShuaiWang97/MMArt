...
\subsection{Art Understanding Datasets}
...
OmniArt~\cite{2017omniart} offers the broadest coverage (432K artworks) but provides only structured metadata rather than natural-language perspectives.

For visual question answering on art, Garcia et al.~\cite{garcia_dataset_2020} and ArtQuest~\cite{bleidt_artquest_2024} introduce benchmarks that probe factual and semantic understanding but again operate within a single question-answering register.
...
PolyArt addresses this gap by providing four complementary perspectives — narrative, formal, emotional, and historical — simultaneously for the same 74,234 paintings, with a unified harmonized caption as a fifth field.
...
\toprule
\textbf{Dataset} & \textbf{Size} & \textbf{Narr.} & \textbf{Form.} & \textbf{Emot.} & \textbf{Hist.} & \textbf{KG} & \textbf{Unified} \\
\midrule
...
\midrule
\textbf{PolyArt (ours)}                 & \textbf{74K} & \checkmark & \checkmark & \checkmark & \checkmark & \checkmark & \checkmark \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Multi-Perspective and Knowledge-Enhanced Art Description}

The interpretive structure of PolyArt follows Panofsky's iconological model~\cite{panofsky1955meaning}, which distinguishes pre-iconographic description (subject matter), iconographic analysis (symbolic meaning), and iconological synthesis (cultural significance).
...
PolyArt incorporates both insights: each perspective is assigned to its best-suited model --- GalleryGPT for formal analysis, Qwen2.5-VL-72B~\cite{qwen2vl} for narrative and historical reasoning, Qwen3-VL-8B~\cite{bai2025qwen3vltechnicalreport} grounded in ArtEmis utterances for emotional response --- and the historical perspective uses LightRAG~\cite{guo2024lightragsimplefastretrievalaugmented} with Wikidata-retrieved context, reducing factual hallucination to below 4\% across the dataset.
